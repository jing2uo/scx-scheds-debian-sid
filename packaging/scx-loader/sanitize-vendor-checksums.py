...
def read_files_excluded(copyright):
...
def remove_files_excluded(copyright):
    removed = 0
    for pattern in read_files_excluded(copyright):
...
def main():
    ...
    parser.add_argument(
        "--copyright", type=Path, default=Path("debian/copyright"),
        help="DEP-5 copyright file containing Files-Excluded entries",
    )
    args = parser.parse_args()
    vendor = Path("vendor")
    if not vendor.is_dir():
        return 0
    remove_files_excluded(args.copyright)
    changed = 0
